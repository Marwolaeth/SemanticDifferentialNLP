...
1. Encoder transformers can, in theory, capture the meaning that is incomprehensible for static embeddings.
2. Transformer embeddings vary in their ability to represent the nuances in meaning.
:::

## Choosing the Right Encoder Model: The Experiment

:::{style="color:red;"}
With this in mind, I made an experiment to test the quality of embeddings from different encoder transformer models. The “quality” here boils down to the model's ability to provide embeddings that are similar for similar sentences (phrases, concepts, etc.) and dissimilar otherwise. Our notion of “quality” does not necessary account for the model's performance on various downstream tasks.
:::

To enrich this exploration, I tested both general pretrained models, such as `bert-base-uncased`, and those specifically trained for natural language inference (NLI) tasks. I also employed sentence transformer (ST) models, that are explicitly trained to recognize text sequences with similar meaning. My hypothesis posited that NLI and ST models would outperform their general counterparts in distinguishing between subtly different senses. The experiment involved comparing similarity scores between sentences and concepts, as well as employing zero-shot classification tasks to evaluate how well these models could assess sentiments related to cats.

:::{style="color:red;"}
Throughout this experiment, I use a handy `text` R package by @kjell-etal-text-package-2023, which is designed to be a convenient R wrapper for the `transformers` Python library. This library is developed by the HuggingFace — a … — to provide … . Th `text` package enables R users to plug the state-of-the-art NLP models for their text analysis workflows … .

I also use the `tidyverse` syntax and packages (@tidyverse-package). For similarity matrix visualisation I use the `pheatmap` package by @pheatmap-package.
:::
